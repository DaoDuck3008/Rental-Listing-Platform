...
import {
  ...
  CreateDestinationValidate,
} from "@/schema/Destination.validator";
import { createDestination } from "@/services/destination.api";
...
export default function CreateDestinationPage() {
  const [form, setForm] = useState<CreateDestinationProps>({
    name: "",
    type: "UNIVERSITY",
    province_code: null,
    ward_code: null,
  ...
  return (
    <>
        ...
        <div className="max-w-240 mx-auto">
          ...
          <div className="mb-8">
            <h1 className="text-2xl md:text-4xl font-black text-text-main tracking-tight mb-2">
              Thêm địa danh phổ biến mới
            </h1>
            <p className="text-text-secondary text-base">
              Địa danh này sẽ giúp người dùng tìm kiếm phòng trọ xung quanh dễ dàng hơn.
            </p>
          </div>
                ...
                >
                  ...
                  <div className="absolute inset-0 flex items-center justify-center bg-black/20 group-hover:bg-black/30 transition-all">
                    <div className="bg-white px-6 py-3 rounded-xl shadow-2xl flex items-center gap-3 text-primary font-bold transform transition-transform group-hover:scale-110 active:scale-95">
                      ...
                      <span>
                        {form.latitude ? "Thay đổi vị trí trên bản đồ" : "Chọn địa chỉ chính xác trên bản đồ"}
                      </span>
                    </div>
                  </div>
                  {form.latitude && (
      ...
      {isSubmitting && <LoadingOverlay message="Đang tạo địa danh..." />}
